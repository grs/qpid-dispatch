...
static void qdr_tcp_connection_copy_reply_to(qdr_tcp_connection_t* tc, qd_iterator_t* reply_to)
{
    ...
    qd_log(tcp_adaptor->log_source, QD_LOG_INFO, "reply-to for %s is %s", tc->ingress ? "ingress" : "egress", tc->reply_to);
}

static void qdr_tcp_second_attach(void *context, qdr_link_t *link,
                                  qdr_terminus_t *source, qdr_terminus_t *target)
{
    qd_log(tcp_adaptor->log_source, QD_LOG_INFO, "qdr_tcp_second_attach for %s", qdr_link_name(link));
...
{
    qd_log(tcp_adaptor->log_source, QD_LOG_INFO, "qdr_tcp_push(%i) for %s", limit, qdr_link_name(link));
